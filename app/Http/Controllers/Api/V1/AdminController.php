...
{
    public function users(Request $request) { $users=User::when($request->q,function($q,$v){return $q->where('name','like','%'.$v.'%')->orWhere('phone_number',$v)->orWhere('email',$v);})->latest()->paginate(30); return response()->json(['success'=>true,'data'=>UserResource::collection($users)]); }
    ...
    public function role(Request $request, User $user) { $data=$request->validate(['role'=>'required|integer|between:0,2']); $user->update($data); return response()->json(['success'=>true,'message'=>'تم تحديث دور المستخدم.','data'=>new UserResource($user)]); }
    ...
    public function recentActivity() { return response()->json(['success'=>true,'data'=>['prices'=>\App\Http\Resources\PriceResource::collection(\App\Models\Price::latest()->limit(10)->get()),'reports'=>ReportResource::collection(Report::latest()->limit(10)->get())]]); }
}
